package org.nullapp.sorrowland.core.loop;
...
abstract public class Loop implements Runnable, Initializable {
  ...
  @Override
  public void run() {
    String  executionInfoTemplate = "fps:%.0f, upd:%.0f, lost:%.0f, free:%.0f upd:%.4fs, render:%.4fs, frames:%d, time: %.2fs";
    float   delayed               = 0;
    ...
    while (isRunning) {
      ...
      if (oneSecondTimer >= ONE_NANO_SECOND) {
        ...
        executionInfo = String.format(executionInfoTemplate, counters[0], counters[1], counters[2], counters[3],
            nanoPerUpdate / ONE_NANO_SECOND, loopTimer[2] / ONE_NANO_SECOND, totalFrames, totalTime / ONE_NANO_SECOND);
